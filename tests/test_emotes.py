...
async def test_get_emote_like_default(postgres):
    postgres.cur.fetchall.side_effect = [[get_sql_emoji(123)]]
    rtn = await postgres.get_emote_like(emote_id=123)
...
async def test_get_emote_like_unusable_no_secondary(postgres):
    postgres.cur.fetchall.side_effect = [[get_sql_emoji(123, usable=False)], []]
    rtn = await postgres.get_emote_like(emote_id=123)
    assert rtn is None
...
async def test_case_insensitive_get_emote(
    postgres, side_effect: List[Optional[SQLEmoji]], expected: Optional[SQLEmoji]
):
    side_effect = [None if i is None else [i] for i in side_effect]
    postgres.cur.fetchall.side_effect = side_effect
    rtn = await postgres._case_insensitive_get_emote(
        query_where="", emote_name="test", parameters={}
    )
    assert rtn == expected
...
async def test_case_insensitive_get_emote_never_unusable(
    postgres, emote_pool: List[SQLEmoji], emote_name: str
):
    sql_query = None
    ...
    rtn: Optional[SQLEmoji] = await postgres._case_insensitive_get_emote(
        query_where="", emote_name=emote_name, parameters={}
    )
    ...
    if rtn is None:
        ...
        if rtn.name != emote_name:
            assert not any(
                emote.name == emote_name and emote.usable for emote in emote_pool
            )


@pytest.mark.asyncio
@given(
    sqlemoji_strategy(emote_hash=sampled_from(["1", "2", "3", "4"])),
    lists(sqlemoji_strategy(emote_hash=sampled_from(["1", "2", "3", "4"]))),
    booleans(),
)
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
async def test_get_emote_like(
    postgres, to_find: SQLEmoji, emote_pool: List[SQLEmoji], add_to_pool: bool
):
    sql_query = None
    parameters_ = {}
    ...
    if not add_to_pool:
        ...
        assert not any(
            emote.usable
            for emote in emote_pool
            if emote.emote_hash == to_find.emote_hash
        )
    else:
