...
class GuildSettingsMixin(_PostgresConnection):
    ...
    async def get_guild_settings(self, guild_id: Union[Guild, int]) -> Optional[GuildSettings]:
        await self.cur.execute(
            "SELECT guild_id, prefix, nitro_role, boost_channel, boost_role, audit_channel, enable_stickers, enable_nitro, enable_replies, enable_masked_links, is_alias_server, locale, enable_pings, max_guildwide_emotes, enable_user_content, enable_personas, enable_dashboard_posting FROM guild_settings WHERE guild_id=%(guild_id)s",
