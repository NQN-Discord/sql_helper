...
class PremiumUser(
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if kwargs:
            kwargs["last_charge_date"] = (
                kwargs.get("last_charge_date") and kwargs["last_charge_date"].date()
            )
    ...
    def should_have_premium(self, discord_requirement: bool = True) -> bool:
        return (
            (self.discord_id or not discord_requirement)
            and self.last_charge_status in ALLOWED_LAST_CHARGE_STATUSES
            and self.last_charge_date + relativedelta(months=1, days=5) >= date.today()
        )
